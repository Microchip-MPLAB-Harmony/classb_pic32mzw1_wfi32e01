...
| CLASSB_CLOCK_TMR1_CLK_FREQ | TMR1 clock frequency. |
| CLASSB_CLOCK_TEST_TMR1_CYCLES | Duration of the CPU clock test. |
...
This function is called from the '_on_bootstrap' function which is executed after a reset, suring startup , before `main()`.
...
This self-test internally call `CLASSB_CPURegistersTest` function to check the processor core registers of the CPU, to detect stuck-at faults.
...
This self-test uses TMR1 and SysTick to measure the CPU clock frequency. The TMR1 is clocked at 32768 Hz from
the external crystal and CPU clock can be from any other high frequency oscillator. If the CPU clock frequency is within
specified error limit, it returns PASS. The test duration is defined by the value of tmr1_cycles. The TMR1 is
...
an external 32.768 kHz accurate crystal connected to the microcontroller.
...
This self-test configures the EVIC, the TMR1 and the TMR2 peripheral
...
time period. It reports a PASS if the total number of interrupts
generated by the TMR2 is less than the specified upper limit and greater than one. 
...
decided by the application, it is the responsibility of the caller to configure the IO pin direction and drive
...
Configure WDT-timeout period, for start-up self test

**Description**

This function is being called from `_on_bootstrap()` function to configure WDT-timeout period for self-test.
